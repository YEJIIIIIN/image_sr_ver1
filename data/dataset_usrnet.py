...
class DatasetUSRNet(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.scales = opt['scales'] if opt['scales'] is not None else [2,4]
        self.sf_validation = opt['sf_validation'] if opt['sf_validation'] is not None else 2
        #self.kernels = hdf5storage.loadmat(os.path.join('kernels', 'kernels_12.mat'))['kernels']
        self.kernels = loadmat(os.path.join('/home/ewha/KAIR-master/kernels', 'kernels_12.mat'))['kernels']  # for validation
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            W = img_H.shape[1]
            crop_size = 96
            r_h = random.randint(0, max(0, H - crop_size))
            r_w = random.randint(0, max(0, W - crop_size))
            img_H = img_H[r_h:r_h + crop_size, r_w:r_w + crop_size, :]
            #####################################################################
