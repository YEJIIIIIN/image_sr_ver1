...
def init_weights(net, init_type='xavier_uniform', init_bn_type='uniform', gain=1):
    ...
    def init_fn(m, init_type='xavier_uniform', init_bn_type='uniform', gain=1):
        ...
        if classname.find('Conv') != -1 or classname.find('Linear') != -1:
            ...
            if init_type == 'normal':
                init.normal_(getattr(m, weight_attr).data, 0, 0.1)
                getattr(m, weight_attr).data.clamp_(-1, 1).mul_(gain)

            elif init_type == 'uniform':
                init.uniform_(getattr(m, weight_attr).data, -0.2, 0.2)
                getattr(m, weight_attr).data.mul_(gain)

            elif init_type == 'xavier_normal':
                init.xavier_normal_(getattr(m, weight_attr).data, gain=gain)
                getattr(m, weight_attr).data.clamp_(-1, 1)

            elif init_type == 'xavier_uniform':
                init.xavier_uniform_(getattr(m, weight_attr).data, gain=gain)

            elif init_type == 'kaiming_normal':
                init.kaiming_normal_(getattr(m, weight_attr).data, a=0, mode='fan_in', nonlinearity='relu')
                getattr(m, weight_attr).data.clamp_(-1, 1).mul_(gain)

            elif init_type == 'kaiming_uniform':
                init.kaiming_uniform_(getattr(m, weight_attr).data, a=0, mode='fan_in', nonlinearity='relu')
                getattr(m, weight_attr).data.mul_(gain)

            elif init_type == 'orthogonal':
                if classname=="Conv2dReLU":
                    for submodule in m.children():
                        if isinstance(submodule, nn.Conv2d):
                            # Apply orthogonal initialization to Conv2d weight
                            init.orthogonal_(submodule.weight.data, gain=gain)

                            # If Conv2d has bias, initialize it
                            if submodule.bias is not None:
                                submodule.bias.data.zero_()
                else:
                    init.orthogonal_(getattr(m, weight_attr).data, gain=gain)
            ...
            elif getattr(m, bias_attr) is not None:
                getattr(m, bias_attr).data.zero_()
